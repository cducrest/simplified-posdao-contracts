...
contract BlockRewardAuRaBase is UpgradeableOwned, IBlockRewardAuRa {
    ...
    function initialize(address _prevBlockReward) external {
        require(_getCurrentBlockNumber() == 0 || msg.sender == _admin());
        require(!isInitialized());
        require(_prevBlockReward != address(0));
    ...
    function isInitialized() public view returns(bool) {
        return _prevBlockRewardContract != IBlockRewardAuRa(0);
    }
    ...
    uint256 internal constant EXTRA_RECEIVER_QUEUE_LIMIT = 100;
        ...
        returns(address[] memory receivers, uint256[] memory rewards)
    {
        ...
        if (extraLength > EXTRA_RECEIVER_QUEUE_LIMIT) {
            extraLength = EXTRA_RECEIVER_QUEUE_LIMIT;
        }
